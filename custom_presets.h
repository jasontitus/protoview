...
/* Unified US TPMS preset: 20 KBaud, 2FSK, 34.9 kHz deviation, 325 kHz BW.
 * Covers GM/Schrader (~20k, ~25-28 kHz dev), Ford (~20k), Stellantis,
 * and Toyota PMV-107J (~10k, ~35 kHz dev).
 * Uses 20 kBaud data rate (wide enough for fast protocols, still captures
 * slower Toyota) with Toyota's 34.9 kHz deviation (wider discriminator
 * still demodulates narrower-deviation signals). */
static uint8_t protoview_subghz_tpms_us_fsk_async_regs[][2] = {
    /* GPIO GD0 */
    ...
    {CC1101_MDMCFG3, 0x93}, // Data rate 20kBaud
    {CC1101_MDMCFG4, 0x59}, // Rx bandwidth filter 325 kHz
    {CC1101_DEVIATN, 0x43}, // Deviation 34.9 kHz
